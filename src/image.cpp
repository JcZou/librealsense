...
#pragma pack(pop)
    ...
    template<rs2_format FORMAT> void unpack_yuy2(byte * const d[], const byte * s, int width, int height)
    {
        ...
        auto src = const_cast<uint8_t *>(reinterpret_cast<const uint8_t *>(s));
        auto dst = reinterpret_cast<uint8_t *>(d[0]);

        cv::Mat yuv(height, width, CV_8UC2, src);
        cv::Mat out;

        switch ( FORMAT ) {
        case RS2_FORMAT_Y8:
            out = cv::Mat(height, width, CV_8UC1, dst);  
            cv::cvtColor(yuv , out, CV_YUV2GRAY_YUY2);
            break;
        case RS2_FORMAT_Y16:
            out = cv::Mat(height, width, CV_16UC1, dst);  
            cv::cvtColor(yuv , out, CV_YUV2GRAY_YUY2);
            break;
        case RS2_FORMAT_RGB8:
            out = cv::Mat(height, width, CV_8UC3, dst);  
            cv::cvtColor(yuv , out, CV_YUV2RGB_YUY2);
            break;
        case RS2_FORMAT_RGBA8:
            out = cv::Mat(height, width, CV_8UC4, dst);  
            cv::cvtColor(yuv , out, CV_YUV2RGBA_YUY2);
            break;
        case RS2_FORMAT_BGR8:
            out = cv::Mat(height, width, CV_8UC3, dst);  
            cv::cvtColor(yuv , out, CV_YUV2BGR_YUY2);
            break;
        case RS2_FORMAT_BGRA8:
            out = cv::Mat(height, width, CV_8UC4, dst);  
            cv::cvtColor(yuv, out, CV_YUV2BGRA_YUY2);
            break;
        default:
            std::runtime_error("Invalid data format");
        }
    }
